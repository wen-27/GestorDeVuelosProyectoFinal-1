...
public sealed class UpdatePersonalUseCase
{
    ...
    public async Task ExecuteAsync(int id, int personId, int positionId, int? airlineId, int? airportId, DateTime hireDate, bool isActive)
    {
        var staff = await _repository.GetByIdAsync(PersonalId.Create(id))
            ?? throw new InvalidOperationException($"No se encontró el empleado con ID {id}.");

        var duplicate = await _repository.GetByPersonIdAsync(PeopleId.Create(personId));
        if (duplicate is not null && duplicate.Id?.Value != id)
            throw new InvalidOperationException($"La persona con ID {personId} ya está vinculada a otro empleado.");

        await EnsureReferencesExistAsync(personId, positionId, airlineId, airportId);

        staff.Update(personId, positionId, airlineId, airportId, hireDate, isActive);
