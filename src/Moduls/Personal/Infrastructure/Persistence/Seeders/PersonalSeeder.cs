...
public sealed class PersonalSeeder
{
    ...
    {
        var people = await _context.Persons.ToDictionaryAsync(x => x.DocumentNumber, x => x.Id, StringComparer.OrdinalIgnoreCase);
        ...
        {
            if (!people.TryGetValue(item.DocumentNumber, out var personId))
                continue;
