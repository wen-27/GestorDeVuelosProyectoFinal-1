...
public sealed class PersonalRepository : IPersonalRepository
{
    ...
    private async Task EnsureNoFutureFlightsAsync(IEnumerable<int> staffIds)
    {
        ...
        var hasFutureFlights = await (
            from assignment in _context.FlightAssignmentStaffReferences.AsNoTracking()
            join flight in _context.FutureFlightReferences.AsNoTracking() on assignment.FlightId equals flight.Id
            where staffIdList.Contains(assignment.StaffId) && flight.DepartureTime > DateTime.UtcNow
