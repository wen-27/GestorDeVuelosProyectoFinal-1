...
public sealed class AirlinesName
{
    ...
    public static AirlinesName Create(string value)
    {
        ...
        if (value.Length > 100 || value.Length < 2)
            throw new ArgumentOutOfRangeException(nameof(value), "El nombre de la aerolínea no puede tener más de 100 caracteres y menos de 2.");
        
        if (value.All(char.IsDigit))
            throw new ArgumentException("El nombre de la aerolínea no puede contener solo números", nameof(value));

        return new AirlinesName(value.Trim());
    }
}
