...
public sealed class CreatePersonEmailUseCase
{
    private readonly IPersonEmailsRepository _repository;
    private readonly AppDbContext _context;
    private readonly IUnitOfWork _unitOfWork;

    public CreatePersonEmailUseCase(
    ...
    public async Task ExecuteAsync(int personId, string emailUser, int emailDomainId, bool isPrimary)
    {
        await EnsureForeignKeysExistAsync(personId, emailDomainId);
        await EnsurePrimaryRuleAsync(personId, isPrimary, currentId: null);
        await EnsureDuplicateEmailAsync(personId, emailUser, emailDomainId, currentId: null);

        var personEmail = PersonEmail.Create(personId, emailUser, emailDomainId, isPrimary);

        await _repository.SaveAsync(personEmail);
